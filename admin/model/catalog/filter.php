...
class ModelCatalogFilter extends Model {
  ...
  public function addOption($data) {
	...
	if (isset($data['filter_group_id'])) {
		foreach ($data['filter_group_id'] as $filter_group_id) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group_option_to_filter_group` SET option_id = '" . (int)$option_id . "', filter_group_id = '" . (int)$filter_group_id . "'");
		}
	}
  ...
  public function editOption($option_id, $data) {
	...
	$this->db->query("DELETE FROM `" . DB_PREFIX . "filter_group_option_to_filter_group` WHERE option_id = '" . (int)$option_id . "'");
  ...
  public function getTotalFiltersByFilterGroupId($filter_group_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "filter_group_option_to_filter_group` WHERE filter_group_id = '" . (int)$filter_group_id . "'");
  ...
  public function getOptionFilterGroups($option_id) {
   $filter_group_id = array();

   $query = $this->db->query("SELECT c.filter_group_id AS filter_group_id FROM `" . DB_PREFIX . "filter_group` c LEFT JOIN `" . DB_PREFIX . "filter_group_option_to_filter_group` cotc ON (c.filter_group_id = cotc.filter_group_id) WHERE cotc.option_id = '" . (int)$option_id . "'");
  ...
  public function getOptions() {
    ...
    foreach ($option_query->rows as $option) {
	  ...
	  $filter_group_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "filter_group` c LEFT JOIN `" . DB_PREFIX . "filter_group_option_to_filter_group` cotc ON (c.filter_group_id = cotc.filter_group_id) LEFT JOIN `" . DB_PREFIX . "filter_group_description` cd ON (cd.filter_group_id = cotc.filter_group_id) WHERE cotc.option_id = '" . (int)$option['option_id'] . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
      ...
      $option_data[] = array(
		...
		'filter_group'  => $filter_group_query->rows,
  ...
  public function createTables() {
	  ...
	  CREATE TABLE IF NOT EXISTS `filter_group_option_to_filter_group` (
        `option_id` int(11) NOT NULL,
        `filter_group_id` int(11) NOT NULL,
        PRIMARY KEY  (`filter_group_id`,`option_id`)
      ) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
	  
	  CREATE TABLE IF NOT EXISTS `filter_group_to_category` (
        `filter_group_id` int(11) NOT NULL,
    ...
    public function deleteTables() {
		...
		$sql = "
		  ...
		  DROP TABLE IF EXISTS `filter_group_option_to_filter_group`;
		  
		  DROP TABLE IF EXISTS `filter_group_to_category`;
