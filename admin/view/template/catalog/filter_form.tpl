    ...
    <form id="form" action="<?php echo $action; ?>" method="post">
      <div>
        <table class="form">
          ...
          <tr>
            <td><?php echo $entry_filter_groups; ?></td>
            <td><div class="scrollbox">
                  <?php $class = 'odd'; ?>
                  <?php foreach ($filter_groups as $filter_group) { ?>
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                  <div class="<?php echo $class; ?>">
                    <?php if (in_array($filter_group['filter_group_id'], $option_filter_group)) { ?>
                    <input type="checkbox" name="filter_group_id[]" value="<?php echo $filter_group['filter_group_id']; ?>" checked="checked" />
                    <?php echo $filter_group['name']; ?>
                    <?php } else { ?>
                    <input type="checkbox" name="filter_group_id[]" value="<?php echo $filter_group['filter_group_id']; ?>" />
                    <?php echo $filter_group['name']; ?>
                    <?php } ?>
                  </div>
                  <?php } ?>
