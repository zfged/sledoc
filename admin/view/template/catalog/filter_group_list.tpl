      ...
      <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
        <table class="list">
          ...
          <tbody>
            ...
            <tr>
              <td style="text-align: center;"><?php if ($filter_group['selected']) { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $filter_group['filter_group_id']; ?>" checked="checked" />
                <?php } else { ?>
                <input type="checkbox" name="selected[]" value="<?php echo $filter_group['filter_group_id']; ?>" />
                <?php } ?></td>
              <td class="left"><?php echo $filter_group['name']; ?></td>
			  <td class="left">
			  <?php foreach ($filter_group['categories'] as $category) { ?>
				   <span class="cat"><?php echo $category['name']; ?></span>
			  <?php } ?>
			  </td>
              <td class="right"><?php echo $filter_group['sort_order']; ?></td>
              <td class="right"><?php foreach ($filter_group['action'] as $action) { ?>
