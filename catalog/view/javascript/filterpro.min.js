...
function success(g, b) {
	...
	var $h1 = $('#container h1:eq(0)');
	var h1_text = $h1.text().replace(/\s\–\sстраница\s[0-9]+$/, '');
	var $title = $(document).find('title');
	var title_text = $title.text().replace(/\s\–\sстраница\s[0-9]+.*/, '').replace(/\s\-\ssledoc\.com\.ua$/, '');
    var page = parseInt($("#filterpro_page").val());
        	
	if (page > 1) {
		h1_text = h1_text + ' – страница ' + page;
		title_text = title_text + ' – страница ' + page;
	}
			
	$h1.text(h1_text);
	$title.text(title_text + ' - sledoc.com.ua');
